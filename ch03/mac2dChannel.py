"""
Channel problem, MAC FD(FV) on a uniform staggered grid
...
# ------------------------------ global variables------------------------------
### problme state
subFolder = "results_channel/"
problemTitle = "mac_channel"
numUDS = 5
titleUDS = ("stream","vortex","Yi","k","epsi")
global tecFileID

nxcv = int(input("Please enter number of CV x direction (nxcv=75): "))
nycv = int(input("Please enter number of CV y direction (nycv=15): "))

### geometry setting
xwest = 0.; xeast = 1.; ysouth = 0.; ynorth = 0.2
...
if __name__ == "__main__":
    ...
    while curTime<t_end:
        ...
        i = 1
        for j in range(1,nycv+1):    
            m1 = m1 + 0.5 * (U[i][j] + U[i-1][j]) * yh
        i = nxcv
        for j in range(1,nycv+1):    
            m2 = m2 - 0.5 * (U[i][j] + U[i-1][j]) * yh 
        massIn.append(m1)
        massOut.append(m2)         
        if istep%ntsave == 0:
    ...
    userContour2D(xNode, yNode, UDS[:,:,0], figname, figtitle, [8,2.5])
    ...
    plt.plot(iterCount, massIn, 'b', label = "flow in")
    plt.plot(iterCount, massOut, 'r', label = "flow out")
    plt.legend(fontsize=20)
    plt.xlabel('time (s)', fontsize=20)
    plt.ylabel('mass flow rate', fontsize=20)
    figname = subFolder + 'massflow_in_out.jpg'
    plt.savefig(figname, format='jpg', dpi=600, bbox_inches='tight')
    plt.show()
