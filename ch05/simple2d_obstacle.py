...
CAES description: channel flow with obstacle  
"""
...
# NOTE: the solver "lgmres" cannot be used for this case

# ------------------------------ global variables------------------------------
### problme state
subFolder = "results_case3/"
problemTitle = "channel_obstacle"
isAxisXSymmetry = 0
isUnsteady = 0

### geometry setting
xwest = 0.; xeast = 0.5; ysouth = 0.; ynorth = 0.2
nxcv = int(input("Please enter number of CV x direction (nxcv=50): "))
nycv = int(input("Please enter number of CV y direction (nycv=20): "))

### physical setting
RE = 200.; rho_ref = 1.; uIN = 1.
...
# save y-ux profile
    ...
    tmpData = np.hstack((yNode.reshape(-1, 1),UX[int(nxcv*0.2)].reshape(-1, 1)))
    tmpData = np.hstack((tmpData,UX[int(nxcv*0.4)].reshape(-1, 1)))
    tmpData = np.hstack((tmpData,UX[int(nxcv*0.6)].reshape(-1, 1)))
    tmpData = np.hstack((tmpData,UX[int(nxcv*0.8)].reshape(-1, 1)))
...
# save 2d field results: PSI
    ...
    userContour2D(xNode, yNode, UDS[:,:,0], fullname, figtitle, [8,3])
